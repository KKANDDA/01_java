...
public class User {
    ...
    /*
    * 생성자의 작성 위치
    * 작성 위치는 문법상으로는 클래스 내부에 작성하면 되지만, 통상적으로 필드 선언부와 메소드 선언부 사이에 작성하는게 관례이다.
    *
    * 생성자의 사용 목적
    * 1. 인스턴스 생성 시점에 수행할 명령이 있는 경우 사용한다.
    * 2. 애개변수 있는 생성자의 경우 매개변수로 전달 받은 값으로 필드를 초기화 하며 인스턴스를 생성할 목적으로 주로 사용된다.
    ...
    * 2. 매가변수 있는 생성자
    * 기본생성자는 자바 컴파일러가 자동으로 추가해 주는 구문이지만 명시적으로 작성할 수도 있다.
    * 매개변수 있는 생성자가 한 개라도 존재하는 경우 기본 생성자를 자동으로 추가해주지 않기 때문에
    * 기본생성자가 필요한 경우에는 반드시 명시적으로 기술해 주어야 한다.
    *
    * */
    ...
    public User(String id, String pwd) {
        this.id = id;
        this.pwd = pwd; // 알트 + 인설트 > 구조 > 선택
    }

    // 전체 필드 초기화 생성자
    public/*매소드랑 비슷한데 타입이 없다!!!! */ User(String id, String pwd, String name, Date enrollDate  /*매개변수*/){
        this.id = id;
        this.pwd = pwd;
        this.name = name;
        this.enrollDate = enrollDate; // 매개변수가 있으면 필드에 있는 변수들..
    }
