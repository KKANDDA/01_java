...
public class Application {
    public static void main(String[] args) {
        ...
        * 3. 리턴값으로 사용할 수 없다.(서로 다른 자료형)
        *
        * */

        /*
        * 클래스명 변수명 = new 클래스명();
        * 지금깢 ㅣ사용한 이 구문은 객체(instance)를 생성하는 구문이다.
        * 사용자 정의의 자료형인 클래스를 이용하기 위해서는 new 연산자로 heap 영역에 할당을 해야한다.
        * 잭체를 생성하게 되면 클래스에 정의한 필드와 메소드 대로 객체가 생성된다.
        * 아이디, 비밀번호, 이름, 나이, 성별, 취미를 연속된 메모리 주소에서 사용하도록 heap 에 공간을 만들었다.
        * */

        Member member = new Member();
        // 클래스는 역할 // Member (요리사)              //  직업이라는 클래스
        // 객체는 구현   // 서현준 (요리하는 직원 서현준)   //  그 직업을 가진 객체

        /*
        * 필드에 접근하기 위해서는 레퍼런스변수명.필드명() 으로 접근한다.
        * '.' 은 참조연산자라고 하는데, 레퍼런스변수(사용자정의 타입, 레퍼런스 타입 다 같은 말..)가 참조하고 있는 주소로 접근한다는 의미를 가진다. 각 공간은 필드명으로 접근한다.
        * */
