...
public class Application {
    public static void main(String[] args) {
        ...
        /*
        * 객체 지향 프로그래밍이란?
        * 현실세계에 모든 사건(event)은 객체와 객체의 상호작용에 의해 일어난다는 세계관을 프로그램을 만들 때 이용하여 새로운 세계를 창조하는 방법론이다.
        * 현실세계와 닮아있지만, 다른 점은 객체간의 상호작용을 한다는 사실은 현실 세게와 동일하지만 의인화 기법이 적용되어
        * 현실세계에서 불가능한 무생물이나 개념 같은 존재들도 하나의 주체로 본인의 상태를 스스로 제어하고 행동하도록 한다는 것이 다른 점이다.
        ...
        * 2. 자동차는 시동걸기, 앞으로 가기 , 멈추기, 시동 끄기
        * 3. 자동차는 처음에 멈춘 상태(시동이 꺼진 상태)로 대시한다.
        * 4. 카레이서는 먼저 다동차에 시동을 건다. 이미 걸려있는 경우는 다시 걸 수 없다.
        * 5. 카레이서가 엑셀을 밟으면 시동이 걸린 상태일 경우 10km/h 씩 증가하며 앞으로 나간다.
        * 6. 자동차가 달리는 중인 경우 브레이크를 밟으면 자동차의 시속은 0으로 떨어지며 멈춘다.
        * 7. 브레이크를 밟을 때 자동차가 달리는 중이 아니라면 아미 멈춰있는 상태라고 안내한다.
        ...
        * */
        ...
        while(true){
            ...
            switch(no){
                ...
                default:
                    System.out.println("잘 못된 번호를 선택하셨습니다."); break;
            }
        }
